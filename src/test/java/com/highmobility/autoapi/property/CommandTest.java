...
/*        // want to test parsing into the command. values are tested in property test.
        ...
        // failure in property
        assertTrue(command.getOpenPercentage().getFailureComponent().getFailureReason() == PropertyFailure.Reason.RATE_LIMIT);
        assertTrue(command.getOpenPercentage().getValue() == null);
        assertTrue(command.getConvertibleRoofState().getFailureComponent().getFailureReason() == PropertyFailure.Reason.EXECUTION_TIMEOUT);
    ...
    @Test public void basePropertiesArrayObjectReplaced() {
        ...
        for (Property property : command.getProperties()) {
            if (property instanceof Property) {
                if (((Property) property).getValueClass() == DashboardLight.class) {
                    found = true;
                }
            }
            break;
