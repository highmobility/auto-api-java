...
public class TextInputTest {
    @Test public void textInputTest() throws CommandParseException, UnsupportedEncodingException {
        byte[] waitingForBytes = Bytes.bytesFromHex("004400010003796573");
        byte[] bytes = new TextInput("yes").getBytes();
        assertTrue(Arrays.equals(waitingForBytes, bytes));
    }

    @Test public void textInputOneLetter() throws CommandParseException, UnsupportedEncodingException {
        byte[] waitingForBytes = Bytes.bytesFromHex("00440001000179");
        byte[] bytes = new TextInput("y").getBytes();
        assertTrue(Arrays.equals(waitingForBytes, bytes));
    }

    @Test public void textInputNoLetters() throws CommandParseException, UnsupportedEncodingException {
